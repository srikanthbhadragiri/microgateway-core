...
function serializeLogRecord(level, configLevel, obj, text) {
  ...
  if (typeof obj === 'string') {
    ...
    if (key === 'req') {
      ...
      if (req) {
        ...
        if( !record.clientIP ){
          record.clientIP = (req.socket ? req.socket.remoteAddress : '');
        }
      }
    } else if (key === 'res') {
      const res = obj[key];
      if (res) {
        ...
        let peerdata = res.socket._peername.address.split(':');
        if( !record.clientIP ) {
      ...
      if (err) {
        ...
        record.componentName = err.component || '';
      }
    } else if (key === 'stats') {
      const stats = obj[key];
      if (stats) {
  ...
  const ClientIp = record.clientIP ? record.clientIP.replace('::ffff:','') : '';
  const ClientId = '';
  const MessageId = '';
