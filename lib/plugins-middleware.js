...
module.exports = function(plugins) {
    ...
    return function(sourceRequest, sourceResponse, next) {
        ...
        if (configService.get().edgemicro.maxHttpHeaderSize && len > configService.get().edgemicro.maxHttpHeaderSize) {
            ...
            logger.error({req: sourceRequest, res: sourceResponse, err: err,component:'plugins-middleware'},'header length more than allowed size');
            //opentracing
...
function getTargetRequest(sourceRequest, sourceResponse, plugins, startTime, correlation_id, cb) {
    ...
    targetRequest.on('error', function(err) {
        ...
        async.series(getPluginHooksForEvent('error', {
            ...
            function(e) {
                if (e) {
                    logger.error({req: sourceRequest, res: sourceResponse, err: e,component:'plugins-middleware'});
                    //opentracing
...
function handleTargetResponse(targetRequest, targetResponse, options, cb) {
    ...
    async.series(
        ...
        function(err, results) {
            if ( err ) logger.error({req: sourceRequest, res: sourceResponse, err: err,component:'plugins-middleware'},"");
            if (sourceResponse.finished || sourceResponse.headersSent) {
                logger.error({req: sourceRequest, res: sourceResponse, err: err,component:'plugins-middleware'},"response finished before work can be done");
                return;
            }
            if (err) {
                //opentracing
    ...
    sourceRequest.on('end', () => {
        onend_request_handlers(empty_buffer,
            function(err, result) {
                ...
                //
                if (err) {
                    return logger.error({req: sourceRequest, res: sourceResponse, err: err,component:'plugins-middleware'});
                }
...
function _subscribeToResponseEvents(plugins, sourceRequest, sourceResponse, targetRequest, targetResponse, correlation_id, start, cb) {
    ...
    ondata_response_transform.on('end', () => {
        onend_response_handlers(empty_buffer,
            function(err, result) {
                if (err) {
                    logger.error({req: sourceRequest, res: sourceResponse, err: err,component:'plugins-middleware'});
                }
...
function getPluginHookForEvent(plugin, type, options) {
    ...
    return (data, cb) => {
        ...
        try {
            ...
            logger.error({req: options.sourceRequest, res: options.sourceResponse, err: handlerException,component:'plugins-middleware'}, plugin.id + " handler threw an exception")
            cb(handlerException)
        }
